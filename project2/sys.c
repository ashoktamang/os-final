...
/*
 * OS Project 2 Part 2 Setting two variables for custom system calls.
 * Symbols to be extracted for logging purposes; Kernel variables.
 */
unsigned int my_accumulator = 0;
...
SYSCALL_DEFINE2(sys_my_set_state, char *the_string, int accumulator)
{
	if (accumulator < 0) {
 		return -EINVAL;
 	}
...
SYSCALL_DEFINE2(sys_my_get_and_sum, int *my_value, int inc_value)
{
	if (inc_value < 0) {
 		return -EINVAL;
 	}
...
SYSCALL_DEFINE2(sys_my_get_string, char *buf, int bsize)
{
